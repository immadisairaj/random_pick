  ...
  group('number range initialization', () {
    group('min and max values', () {
      ...
      test('min is equal to max', () {
        ...
        expect(() => NumberRange(max: 0),
            throwsA(const TypeMatcher<ArgumentError>()));
        // use custom min value and min is equal to max
        expect(() => NumberRange(min: 3, max: 3),
            throwsA(const TypeMatcher<ArgumentError>()));
      });
      ...
      test('difference between min and max', () {
        // range is less than 2^32
        expect(() => NumberRange(min: 1, max: 1 << 32), returnsNormally);
        // range is greater than or equal to 2^32
        expect(() => NumberRange(max: 1 << 32),
            throwsA(const TypeMatcher<ArgumentError>()));
      });
    });
    group('error messages', () {
      test('values error', () {
        expect(
            ...
            throwsA(isArgumentError.having((e) => e.message,
                'values error message', 'min must be less than max')));
      });
      test('difference error', () {
        expect(
            () => NumberRange(max: 1 << 32),
            throwsA(isArgumentError.having(
                (e) => e.message,
                'difference error message',
                'the total list size must be less than 2^32,'
                    ' i.e. 4,294,967,296')));
      });
    });
  });
